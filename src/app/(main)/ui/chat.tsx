...
export function Chatbot() {
  ...
  return (
    <>
      ...
      <FloatingWhatsAppBtn />
      <div
        className={`fixed bottom-[calc(3rem+1.5rem)] right-0 mr-4 bg-white px-4 pt-6 pb-2 rounded-lg border border-[#e5e7eb] w-[340px] h-[400px] flex flex-col transition-all duration-300 ease-in-out z-40 shadow-lg ${
          isOpen
        ...
        <div className="flex-1 overflow-auto pr-2" id="chats-box">
          {messages.map((message, index) => {
            return message.user ? (
              ...
              <AIMessage key={index} msg={message.text} />
            );
          })}
...
interface HeaderProps {
  setMesssages: React.Dispatch<
    React.SetStateAction<{ user: boolean; text: string }[]>
  >;
}
    ...
    <div className="flex flex-col space-y-1.5 pb-4">
      <div className="flex justify-between items-center">
        ...
        <small
          ...
          onClick={() => setMesssages([])}
        >
          Start over
        </small>
      </div>
      <p className="text-xs text-[#6b7280] leading-3">Good to see you!</p>
    </div>
  );
}
...
function ChatInput({ messages, setMessages, setIsLoading }: ChatInputProps) {
          ...
          body: JSON.stringify({
            ...
            messages: [
              ...
              ${messages
                .map(
                  (message) =>
                    `${message.user ? "user: " : "you: "} - ${message.text}`
                )
                .join("\n")}`,
              },
              {
  ...
  return (
    <form
      className="flex items-end space-x-2 pt-2"
...
function FloatingWhatsAppBtn() {
  return (
    <a
      href="https://wa.me/+923281177401"
      target="_blank"
      rel="noreferrer noopener"
      className="fixed bottom-18 right-4 inline-flex items-center justify-center w-10 h-10 rounded-full bg-[#25d366]"
    >
